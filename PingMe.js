//2026/04/16
/*
@Name：PingMe 自动化签到+视频奖励（Loon 版）
@Author：怎么肥事（原作者，Loon 适配：Claude）

Loon 插件请配套使用 PingMe.plugin 文件加载，
本脚本已将 QuantumultX 专属 API 替换为 Loon 对应 API：
  $prefs           -> $persistentStore
  $notify          -> $notification.post
  $task.fetch      -> $httpClient.get（回调式，已包成 Promise）
*/
...
// ---- Loon 专属：$httpClient 是回调式，这里包一层 Promise ----
...
function notifyDone(title, body) {
  // Loon: $notification.post(title, subtitle, body)
  ...
  if (!raw) {
    ...
    if (capture) {
      ...
      function fetchApi(path) {
        return httpGet({ url: buildUrl(path, capture), method: 'GET', headers });
      }
          ...
          return new Promise(resolve => {
            setTimeout(() => {
              ...
              fetchApi('videoBonus').then(res => {
                ...
                msgs.push(`❌ 视频${i}：${(err && err.error) || '请求失败'}`);
                resolve();
              });
